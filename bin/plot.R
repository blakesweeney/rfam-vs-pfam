...
pivoted <- data %>% pivot_longer(!rfam_acc & !description & !source & !rna_type, names_to="stat", values_to="value")
medians <- data %>%
    group_by(source) %>%
    summarise(
...
## Displayed by counts
family_counts <- rfam_rna_type_df %>%
...
alignment_size <- ggplot(data, aes(x=source, y=number_of_columns, color=source, fill=source)) +
	...
	scale_color_manual(values = c(light_pfam, medium_pfam, light_rfam, medium_rfam)) +
	scale_fill_manual(values = c(light_pfam, medium_pfam, light_rfam, medium_rfam)) +
...
rfam_structures_df <- data %>%
 ...
 structures_plot <- ggplot(rfam_structures_df, aes(x=reorder(rna_type, -value), y=value)) +
  	geom_bar(stat="identity") + facet_grid(stat ~ ., scales="free_y") +
  	theme(axis.text.x = element_text(angle = 45, vjust=0.9, hjust=1)) + labs(x="Rfam RNA Type", y="Count")
